...
export default function OrderTrackingPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#fffcf2] flex items-center justify-center text-[#156734]/60 text-xs tracking-widest font-black uppercase font-sans">
  ...
  if (error || !order) {
    return (
      <div className="min-h-screen bg-[#fffcf2] text-[#156734] flex flex-col items-center justify-center p-6 text-center gap-4 select-none font-sans">
        ...
        <p className="text-xs text-[#156734]/70 max-w-xs leading-relaxed font-medium">
          The order ID does not exist or has expired. Please check the URL and try again.
        </p>
        <Link
          href="/menu"
          className="mt-2 inline-flex items-center gap-2 px-5 py-2.5 bg-[#156734] hover:bg-[#0f4d27] text-white text-[10px] font-black tracking-widest uppercase rounded-xl transition-all shadow-md"
  ...
  return (
    <div className="min-h-screen bg-[#fffcf2] text-[#156734] font-sans flex flex-col justify-between p-5 relative overflow-hidden select-none pb-24">
      {/* Background Glow */}
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-[#156734]/5 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <header className="w-full max-w-md mx-auto flex justify-between items-center z-10 py-1">
        <Link href="/menu" className="flex items-center cursor-pointer">
          <span
            className="text-[#156734] font-black text-lg tracking-tight uppercase"
            style={{ fontFamily: "Montserrat, sans-serif" }}
          >
            Vellari
          </span>
        ...
        <span
          className="text-[9px] font-black text-[#156734]/40 tracking-widest uppercase"
          style={{ fontFamily: "Montserrat, sans-serif" }}
        >
      ...
      <div className="max-w-md w-full mx-auto bg-[#fffcf2] border border-[#e5dbb2]/60 rounded-3xl p-5 md:p-6 text-center shadow-[0px_6px_24px_rgba(21,103,52,0.03)] flex flex-col gap-5 z-10 my-4">
        ...
        {isPendingVerification && (
          <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-4 text-center flex flex-col items-center gap-2 animate-pulse">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <p className="text-[9px] text-[#156734]/80 font-semibold leading-relaxed mt-0.5">
        ...
        <div className="flex flex-col items-center gap-3.5 py-1">
          {/* Status Illustration */}
          <div className="w-16 h-16 rounded-full bg-[#156734]/5 flex items-center justify-center border border-[#156734]/10 relative">
            {isPendingVerification && (
              <span className="material-symbols-outlined text-[#156734]/85 text-2xl animate-bounce">pending_actions</span>
            )}
            {isPendingAccept && (
              <span className="material-symbols-outlined text-[#156734]/85 text-2xl animate-spin">hourglass_empty</span>
            )}
            {isAccepted && (
              <span className="material-symbols-outlined text-[#156734] text-2xl animate-pulse">soup_kitchen</span>
            )}
            {isCompleted && (
              <span className="material-symbols-outlined text-[#156734] text-2xl">celebration</span>
            )}
            {isCancelled && (
          ...
          <div className="flex flex-col gap-1">
            <h2 className="text-sm font-black tracking-wider uppercase text-[#156734]" style={{ fontFamily: "Montserrat, sans-serif" }}>
            ...
            <p className="text-[10px] text-[#156734]/60 leading-normal font-medium max-w-xs px-2">
        ...
        <div className="text-left flex flex-col gap-2 text-xs">
          <span className="text-[8px] font-black tracking-widest text-[#156734]/40 uppercase" style={{ fontFamily: "Montserrat, sans-serif" }}>
            ORDER DETAILS
          </span>
          ...
          <div className="bg-white/40 border border-[#e5dbb2]/45 rounded-2xl p-3 flex flex-col gap-2.5">
            ...
            <ul className="flex flex-col gap-1.5 border-b border-[#e5dbb2]/30 pb-2.5">
              {Array.isArray(order.items) &&
                order.items.map((item, idx) => (
                  <li key={idx} className="flex justify-between font-bold text-[#156734] text-xs">
                    <span>
                      <strong className="text-[#156734]/85 font-black mr-2">{item.quantity}x</strong>
                      {item.name}
                    </span>
                  </li>
                ))}
            ...
            <div className="flex justify-between items-center text-xs font-black tracking-wide" style={{ fontFamily: "Montserrat, sans-serif" }}>
              <span className="text-[#156734]/60">TOTAL BILL</span>
              <span className="text-[#156734] text-sm">AED {Number(order.total).toFixed(2)}</span>
            </div>
          </div>
        </div>
        ...
        {order.order_type === "delivery" && order.address_details && (
          <div className="text-left flex flex-col gap-1.5 text-xs">
            ...
            <p className="text-[#156734]/80 font-bold leading-normal text-xs">{order.address_details}</p>
          </div>
        )}
      </div>
      ...
      <footer className="w-full text-center z-10 py-2 flex flex-col gap-1">
        <p className="text-[9px] text-[#156734]/40 font-bold leading-relaxed" style={{ fontFamily: "Montserrat, sans-serif" }}>
          Need support? Call us: <a href="tel:+97148342856" className="text-[#156734]/60 font-black hover:underline">+971 4 834 2856</a>
        </p>
        <p className="text-[9px] text-[#156734]/30 font-medium">© 2026 Vellari Karama. All rights reserved.</p>
      </footer>
    </div>
  );
}
